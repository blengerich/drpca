...
def read_data(directory, f_name):
    transcript_names = []
    levels = []
    #with open(directory + '/' + f_name, 'r') as csvfile:
...
def get_data_from_dirs():
    ...
    with open("../dfi.json", 'r') as dfi_file:
        dfi_dict = json.load(dfi_file)

    with open("../os.json", 'r') as os_file:
        os_dict = json.load(os_file)

    with open("../dss.json", 'r') as dss_file:
        dss_dict = json.load(dss_file)

    with open("metadata.cart.2017-11-24T18_59_58.545197.json", 'r') as json_file:
    ...
    for i, entry in enumerate(head):
        ...
        if read_all_data:
            ...
            if sample["sample_type"] == "Primary Tumor":
                ...
                case_dfi = []
                try:
                    for x in dfi_dict[case["submitter_id"]]:
                        try:
                            case_dfi.append(int(x))
                        except ValueError:
                            case_dfi.append(-1)
                except KeyError:
                    case_dfi = [-1, -1]
                cases_dfi.append(case_pfi)

                case_dss = []
                try:
                    for x in dss_dict[case["submitter_id"]]:
                        try:
                            case_dss.append(int(x))
                        except ValueError:
                            case_dss.append(-1)
                except KeyError:
                    case_dss = [-1, -1]
                cases_dss.append(case_dss)

                case_os = []
                try:
                    for x in os_dict[case["submitter_id"]]:
                        try:
                            case_os.append(int(x))
                        except ValueError:
                            case_os.append(-1)
                except KeyError:
                    case_os = [-1, -1]
                cases_os.append(case_os)
            elif sample["sample_type"] == "Solid Tissue Normal":
    ...
    print(matches)
    return cases, controls, np.array(matches), expected_transcript_names, cases_pfi, cases_dfi, cases_dss, cases_os, np.array(cases_stages)


cases, controls, matches, transcript_names, cases_pfi, cases_dfi, cases_dss, cases_os, cases_stages = get_data_from_dirs()
...
np.save("cases_dfi", cases_dfi)
np.save("cases_dss", cases_dss)
np.save("cases_os",  cases_os)
